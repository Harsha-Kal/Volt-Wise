...
const PROVIDERS = [
  ...
  { value: "CORE", label: "CORE Electric Cooperative" },
  { value: "United Power", label: "United Power" },
];
...
export default function AccountPage() {
  ...
  const handleProviderUpdate = async () => {
    ...
    try {
      ...
      setProfile(updated);
      setProviderMsg("✓ Provider updated successfully!");
    } catch (e: any) {
      setProviderMsg("❌ Failed to update provider.");
    } finally {
  ...
  const simulatedSavings = shiftedTasks.length * 1.45;
      ...
      <div className="max-w-4xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
          <div className="bg-emerald-500/10 border border-emerald-500/20 rounded-2xl p-6">
            ...
            <p className="text-xs text-emerald-400/60 mt-2">Saved by Volt-Wise shifting your tasks.</p>
          </div>
        ...
        {/* Provider Settings */}
        <div className="bg-neutral-900/50 border border-white/10 rounded-2xl p-6 mb-8">
          <h2 className="text-lg font-medium mb-4">Energy Provider Settings</h2>
          <p className="text-sm text-neutral-400 mb-4">Changing your provider updates the grid rate calculations used on your dashboard and planner.</p>
          <div className="flex gap-3 items-center flex-wrap">
            <select
              value={selectedProvider}
              onChange={(e) => setSelectedProvider(e.target.value)}
              className="bg-black border border-white/10 rounded-lg px-4 py-2 text-sm text-white focus:outline-none focus:border-emerald-500/50 appearance-none"
            >
              {PROVIDERS.map(p => (
            ...
            <button
              ...
              disabled={providerSaving || selectedProvider === profile?.provider}
              className="px-5 py-2 bg-emerald-500/10 hover:bg-emerald-500/20 border border-emerald-500/30 text-emerald-400 rounded-lg text-sm font-medium transition-colors disabled:opacity-50"
            >
              {providerSaving ? "Saving..." : "Save Changes"}
            </button>
            {providerMsg && (
              <span className={`text-sm ${providerMsg.startsWith("✓") ? "text-emerald-400" : "text-red-400"}`}>
                {providerMsg}
              </span>
            )}
          </div>
